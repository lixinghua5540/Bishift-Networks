...
class ShiftNetModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # self.loss_names = ['G_GAN', 'D', 'style', 'content', 'tv']
        ...
        self.netG, self.ng_innerCos_list, self.ng_shift_list = networks.define_G(input_nc,
                                                                                opt.output_nc,
                                                                                opt.ngf,
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(opt.input_nc, opt.ndf,
                                          ...
                                          opt.n_layers_D,
                                          opt.norm, use_sigmoid, opt.use_spectral_norm_D, opt.init_type, self.gpu_ids, opt.init_gain)
        ...
        if self.isTrain:
            ...
            # SSIM LOSS
            self.ssim_crterion = networks.SSIM()
            # PSNR LOSS
            # self.psnr_crterion = networks.PSNR()
    ...
    def set_input(self, input):
        ...
        if self.opt.add_mask2input:
            ...
            real_A = torch.cat((real_A, (~self.mask_global).expand(real_A.size(0), 1, real_A.size(2),
                                                                   real_A.size(3)).type_as(real_A)), dim=1)
    ...
    def set_gt_latent(self):
        if not self.opt.skip:
            if self.opt.add_mask2input:
                # make it 4 dimensions.
                # Mention: the extra dim, the masked part is filled with 0, non-mask part is filled with 1.
                real_B = torch.cat([self.real_B, (~self.mask_global).expand(self.real_B.size(0), 1, self.real_B.size(2),
    ...
    def backward_D(self):
        ...
        if self.opt.gan_type == 'wgan_gp':
            gradient_penalty, _ = util.cal_gradient_penalty(self.netD, real_B, fake_B.detach(), self.device, constant=1,
                                                            lambda_gp=self.opt.gp_lambda)
    ...
    def backward_G(self):
        ...
        self.loss_ssim = 1 - self.ssim_crterion(self.fake_B, self.real_B)
        # self.loss_psnr = self.psnr_crterion(self.fake_B, self.real_B)
        ...
        self.opt.style_weight = 1
        self.opt.content_weight = 1
        self.loss_style = self.opt.style_weight * self.loss_style
        self.loss_content = self.opt.content_weight * self.loss_content
        self.tv_weight = 1
        self.loss_tv = self.tv_weight * self.loss_tv
        self.loss_ssim = 10 * self.loss_ssim
        # self.loss_psnr = -1 * self.loss_psnr
        self.opt.gan_weight = 2
        self.opt.lambda_A = 100

        self.loss_G_GAN = self.opt.gan_weight * self.loss_G_GAN
        self.loss_G_L1 = self.opt.lambda_A * self.loss_G_L1



        self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_tv + self.loss_style + self.loss_content + self.loss_ssim
