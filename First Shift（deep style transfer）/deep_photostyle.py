...
parser.add_argument("--content_image_path", dest='content_image_path',  nargs='?',
                    help="Path to the content image", default='./TestImage/t-r2.png')
parser.add_argument("--style_image_path",   dest='style_image_path',    nargs='?',
                    help="Path to the style image", default='./TestImage/r2.png')
parser.add_argument("--content_seg_path",   dest='content_seg_path',    nargs='?',
                    help="Path to the style segmentation", default='./TestImage/r2-mask.tif')
parser.add_argument("--style_seg_path",     dest='style_seg_path',      nargs='?',
                    help="Path to the style segmentation", default='./TestImage/r2-mask.png')
...
parser.add_argument("--output_image",       dest='output_image',        nargs='?',
                    help='Path to output the stylized image', default="./TestImage/best_stylized.png")
parser.add_argument("--serial",             dest='serial',              nargs='?',
                    help='Path to save the serial out_iter_X.png', default='./TestImage/serial')
...
def main1(i):
    ...
    args.content_image_path = './TestImage/t-' + i + '.png'
    args.serial = './TestImage/out/' + i
    path = './TestImage/out/' + i
...
if __name__ == "__main__":
    ...
    #         continue
    #     else:
    #         img = cv2.imread('./TestImage/real2/' + str(j) + '.png')
    #         if img is not None:
    #             main1(j)
    #         else:
    #             continue
